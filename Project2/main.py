...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_vgg7out = tf.layers.conv2d(inputs=vgg_layer7_out,
                                filters=num_classes, kernel_size=1,
                                strides=1, padding="same",
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    # upsampling vgg layer 7
    layer4_in1 = tf.layers.conv2d_transpose(inputs=conv_1x1_vgg7out,
                                filters=num_classes, kernel_size=4,
                                strides=2, padding="same",
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    # 1x1 convolution for vgg_layer4_out
    layer4_in2 = tf.layers.conv2d(inputs=vgg_layer4_out,
                                        filters=num_classes, kernel_size=1,
                                        strides=1, padding="same",
                                        kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # 1x1 convolution for vgg_layer7_out
    layer3_in1 = tf.layers.conv2d(inputs=vgg_layer3_out,
                                        filters=num_classes, kernel_size=1,
                                        strides=1, padding="same",
                                        kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # upsampling
    layer3_in2 = tf.layers.conv2d_transpose(inputs=layer4_out,
                                filters=num_classes, kernel_size=4,
                                strides=2, padding="same",
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # upsampling
    # nn_last_layer
    output_ = tf.layers.conv2d_transpose(inputs=layer3_out,
                                         filters=num_classes, kernel_size=16,
                                         strides=8, padding="same",
                                         kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                         kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    # tf.Print(output_, [tf.shape(output_)[1:3]])
    return output_
